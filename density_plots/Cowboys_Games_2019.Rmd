---
title: "Density Plots - Dallas Cowboys 2019 Regular Season Completed Pass Locations"
date: Thru Week 13
output: pdf_document
---
...
TEAM_FILTER <- "DAL"
passes <- subset(passes, home_team == TEAM_FILTER | away_team == TEAM_FILTER)
...
passes<- passes %>%
  mutate(team_result = 
           ifelse(
            (home_team == TEAM_FILTER & home_score > away_score) |
            (away_team == TEAM_FILTER & away_score > home_score),
            "Wins",
            "Losses")
  )
...
groups <- sort(unique(as.character(complete_19$team_result)))

for (g in groups) {

  group_passes <- subset(passes_19, team_result == g)
  group_complete <- subset(complete_19, team_result == g)
  group_incomplete <- subset(incomplete_19, team_result == g)
  ...
  # KDE plot of all Passes
  group_passes_kde <- kde2d(group_passes$x, group_passes$y, h = c(5, 5))
  group_passes_grid <- data.frame(with(group_passes_kde, expand.grid(x,y)), as.vector(group_passes_kde$z))
  names(group_passes_grid) <- c("x", "y", "z")
  group_passes_model <- gam(z ~ s(x, y), data = group_passes_grid)
  group_passes$dens <- ramify::clip(predict(group_passes_model,
                                newdata = data.frame(x = group_passes$x, y = group_passes$y)
                                ),0,1)
  median_passes <-  median(group_passes$dens)
  
  group_passes_dens <- ggplot(group_passes, aes(x = x, y = y, color = dens)) +
    stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE,
                 h = c(7.5, 7.5)) +
    scale_fill_gradient2(low = "darkred", high = "darkblue", mid = "white", midpoint = median_passes) +
    labs(x = "Field Width", y = "Field Length",
       color = "Density", fill = "KDE\nLevel") +
    geom_hline(color = "blue", yintercept = 0) +
    annotate("text", -30, 0, vjust = -0.5, label = "LOS") +
    ggtitle(paste("All Passes Density:\n", g, " in 2019", sep = ""))
  print(group_passes_dens)
